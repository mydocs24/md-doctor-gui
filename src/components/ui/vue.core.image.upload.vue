<template>
    <div class="g-core-image-upload-btn">
        <img src="../../assets/camera.svg" class="upload-btn w-100 figure-img rounded-circle img-fluid"
             :alt="text">
        ...
        <form class="g-core-image-upload-form" v-show="!hasImage" method="post" enctype="multipart/form-data"
              ...
              style="display: block; cursor: pointer; position: absolute; left: 0px; top: 0px; width: 100%; height: 100%; opacity: 0; margin: 0; padding: 0; overflow: hidden;">
            <input v-bind:disabled="uploading" v-bind:id="'g-core-upload-input-' + formID" v-bind:name="inputOfFile"
                   type="file" v-bind:accept="inputAccept" v-on:change="change" style="width: 100%; height: 100%; cursor: pointer;">
        </form>
                ...
                <div class="g-crop-image-box">
                    <div class="g-crop-image-principal">
                        <img v-bind:src="image.src"
                             v-bind:style="{ width:image.width + 'px',height: image.height + 'px' }">
                        <div class="select-recorte" v-on:touchstart.self="drag" v-on:mousedown.self="drag"
      ...
      methods: {
        ...
        change (e) {
          ...
          if (extensionsArr.length > 1) {
            let reg = new RegExp('^[' + extensionsArr.join('|') + ']+$', 'i')
            if (!reg.test(fileExt)) {
        ...
        __initCropBox () {
          ...
          let ratioH = this.cropRatio.split(':')[1]
        ...
        resize (e) {
          ...
          new Resize($el, $container, this.cropRatio, e)
        },
